...
st_write(filter(tem.pem.rcl, SITE.CALL == "rspns_t_1"),
         dsn = 'C:/Users/ccarmour.stu/OneDrive - Government of BC/PEM/Deception_AOI/40_accuracy_testing/tem_pem_rcl_rspns_1.shp',
         append = FALSE)
st_write(filter(tem.pem.rcl, SITE.CALL == "rspns_t_2"),
         dsn = 'C:/Users/ccarmour.stu/OneDrive - Government of BC/PEM/Deception_AOI/40_accuracy_testing/tem_pem_rcl_rspns_2.shp',
         append = FALSE)
st_write(filter(tem.pem.rcl, SITE.CALL == "rspns_t_3"),
         dsn = 'C:/Users/ccarmour.stu/OneDrive - Government of BC/PEM/Deception_AOI/40_accuracy_testing/tem_pem_rcl_rspns_3.shp',
         append = FALSE)

## Create unpivoted but still reclassified version for geometric intersections so as not to triplicate polygons
tem.pem.rcl_wide <- tem.pem %>%
  filter(BGC_LBL != "SBSdk") %>%
  mutate(SITES_LBL1_full = case_when(!is.na(SITES_LBL1) ~ str_c(BGC_LBL, SITES_LBL1, sep = "_"), TRUE ~ NA_character_),
         SITES_LBL2_full = case_when(!is.na(SITES_LBL2) ~ str_c(BGC_LBL, SITES_LBL2, sep = "_"), TRUE ~ NA_character_),
         SITES_LBL3_full = case_when(!is.na(SITES_LBL3) ~ str_c(BGC_LBL, SITES_LBL3, sep = "_"), TRUE ~ NA_character_)) %>%
...
int.mle.tem <-st_intersection(mle.pem.rcl, tem.pem.rcl_wide) %>% st_collection_extract(., "POLYGON")

st_write(int.mle.tem, dsn = "C:/Users/ccarmour.stu/OneDrive - Government of BC/PEM/Deception_AOI/40_accuracy_testing/tem_mle_int.shp")
...
area.totals.mt <- full_join(area.tem, area.mle.tem, join_by(rspns_t == rspns_m), keep = TRUE, na_matches = "never") %>%
  ...
  filter(str_detect(BGC_LBL, pattern = "[wp]$", negate = T)) %>%
...
write.csv(area.totals.mt, "C:/Users/ccarmour.stu/OneDrive - Government of BC/PEM/Deception_AOI/area_totals_TEM_MLE.csv", append = FALSE)
...
int.mle.exp <- st_intersection(mle.pem.rcl, exp.pem.rcl) %>% st_collection_extract(., "POLYGON")

st_write(int.mle.exp, dsn = "C:/Users/ccarmour.stu/OneDrive - Government of BC/PEM/Deception_AOI/40_accuracy_testing/exp_mle_int.shp")
...
area.totals.mx <- full_join(area.exp, area.mle.exp, join_by(rspns_x == rspns_m), keep = TRUE, na_matches = "never") %>%
  ...
  mutate(rspns = case_when(!is.na(rspns_m) ~ rspns_m, TRUE ~ rspns_x),
         BGC_LBL = str_split(rspns, pattern = "_", simplify = TRUE)[,1]) %>%
  filter(str_detect(BGC_LBL, pattern = "[p]$", negate = T)) %>%
...
write.csv(area.totals.mx, "C:/Users/ccarmour.stu/OneDrive - Government of BC/PEM/Deception_AOI/area_totals_EXP_MLE.csv", append = FALSE)
